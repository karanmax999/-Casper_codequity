...
export default function SignUpPage() {
  ...
  const handlePasswordSignUp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setSuccessMsg("Registration successful! Check your email for validation link.");
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#000000] p-4 text-foreground selection:bg-accent selection:text-black">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="rounded-lg border border-[#1F1F1F] bg-[#0A0A0A] p-6 shadow-2xl space-y-6">
          {!supabase ? (
            ...
            <>
              ...
              {successMsg && (
                <div className="flex items-center gap-2 rounded-sm border border-[#45f798]/30 bg-[#45f798]/10 p-3 text-xs text-[#45f798]">
                  <AlertCircle className="h-4 w-4 shrink-0" />
        ...
        <p className="text-center text-xs text-zinc-500">
          ...
          <Link href="/sign-in" className="text-[#45f798] hover:underline font-semibold">
            Sign in
          </Link>
        </p>
      </div>
